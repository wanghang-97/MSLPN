...
parser.add_argument('--step_size', type=int, default=25000, metavar='STEPSIZE',
...
def _init_():
    if not os.path.exists('tieredImageNet_checkpoint_five_shot'):
        os.makedirs('tieredImageNet_checkpoint_five_shot')
    if not os.path.exists('tieredImageNet_checkpoint_five_shot/' + args['exp_name']):
        os.makedirs('tieredImageNet_checkpoint_five_shot/' + args['exp_name'])
    if not os.path.exists('tieredImageNet_checkpoint_five_shot/' + args['exp_name'] + '/' + 'models'):
        os.makedirs('tieredImageNet_checkpoint_five_shot/' + args['exp_name'] + '/' + 'models')
    f = open('tieredImageNet_checkpoint_five_shot/' + args['exp_name'] + '/log.txt', 'a')
    print(args, file=f)
    print(u'\r')
    f.close()
_init_()
...
for ep in range(iters, n_epochs):
    ...
    if cond1 or cond2:
        ...
        torch.save(model.state_dict(),
                   'tieredImageNet_checkpoint_five_shot/%s/models/%s_%d_model.pkl' % (args['exp_name'], args['alg'], (ep + 1) * n_episodes))
        ...
        if ep % 100 == 0:
            torch.save(model.state_dict(),
                       'tieredImageNet_checkpoint_five_shot/%s/models/%s_%d_model.pkl' % (
    ...
    if wait > patience and ep > n_epochs:
        break
    result.to_csv('./tieredImageNet_checkpoint_five_shot/val_acc_loss1.csv')